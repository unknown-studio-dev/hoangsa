...
4. Extract frames, then build both montages:
   - `hoangsa-cli media frames <video_path> --output-dir $OUTPUT_DIR/frames`
   - `hoangsa-cli media montage $OUTPUT_DIR/frames --timestamps --output $OUTPUT_DIR/montage.png`
   - `hoangsa-cli media diff $OUTPUT_DIR/frames --output $OUTPUT_DIR/diff-montage.png`
5. Read output files:
