...
Extract `SESSION_DIR` from the JSON output — it is the `dir` field:

```bash
SESSION_DIR=$(echo "$SESSION" | jq -r '.dir')
```

**IMPORTANT:** All subsequent references to `$SESSION_DIR` in this workflow use this extracted value. Do NOT use `$SESSION` (raw JSON) where a directory path is expected.
...
### File tree scan (MANDATORY for path accuracy)

Before generating any plan, scan the actual workspace to ground path references:

```bash
# Get source file tree (excludes hidden dirs, node_modules, target, dist, build)
...
Store the output as `FILE_TREE`. **All paths in `files[]` and `context_pointers[]` MUST reference actual files from this tree** (except CREATE actions for new files — those paths must follow the existing directory structure).

**NEVER invent path segments.** If unsure where a file should go, check the tree for similar files and follow the established pattern.
...
## Step 4b: Resolve paths — MANDATORY

Run path resolution immediately after saving plan.json. This normalizes relative paths to absolute and fuzzy-matches non-existent paths against the actual workspace:

```bash
"$HOANGSA_ROOT/bin/hoangsa-cli" plan resolve "$SESSION_DIR/plan.json"
```

If `fix_count > 0` → review the fixes. The command modifies plan.json in-place and reports what it changed. If any paths could not be resolved (ambiguous or no match), manually fix them before proceeding.
...
```bash
TASK_IDS=$("$HOANGSA_ROOT/bin/hoangsa-cli" plan task-ids "$SESSION_DIR/plan.json")
for TASK_ID in $(echo "$TASK_IDS" | jq -r '.task_ids[]'); do
  "$HOANGSA_ROOT/bin/hoangsa-cli" context pack "$SESSION_DIR" "$TASK_ID"
done
```
