...
| # | Gate | Check |
|---|------|-------|
| 1 | Acceptance | the task's `acceptance` command exits clean |
...
**Chain:** `auto_plate` true → `/hoangsa:plate`; false → show next steps; null → ask once (AskUserQuestion, adapt to `lang`: "Auto-commit after taste passes?" — Always / No), save via `pref set . auto_plate`, proceed accordingly.
