...
use anyhow::Result;
use hoangsa_memory_parse::{LanguageRegistry, watch::Watcher};
    ...
    loop {
        tokio::select! {
            ...
            ev = w.recv() => {
                ...
                for ev in batch {
                    match ev {
                        hoangsa_memory_core::Event::FileChanged { path, .. } => changed.push(path),
                        hoangsa_memory_core::Event::FileDeleted { path, .. } => deleted.push(path),
                        _ => {}
                    }
                }
