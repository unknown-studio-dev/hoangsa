...
use hoangsa_memory_core::{Chunk, ChunkContext, Result, SymbolRef};
use hoangsa_memory_graph::{Graph, Node};
...
/// whose source is [`hoangsa_memory_core::RetrievalSource::Markdown`] or
/// [`hoangsa_memory_core::RetrievalSource::Episodic`] are left alone — there's
/// no file/symbol to enrich against.
pub async fn enrich_chunks(graph: &Graph, chunks: &mut [Chunk]) -> Result<()> {
    for chunk in chunks.iter_mut() {
        use hoangsa_memory_core::RetrievalSource::*;
        match chunk.source {
