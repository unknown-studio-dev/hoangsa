...
///
/// Distinct from [`hoangsa_memory_core::MemoryKind`] (the five-class taxonomy) —
...
fn pick_entry(entries: &[String], query: &str) -> Result<usize> {
    ...
    if needle.is_empty() {
        return Err(hoangsa_memory_core::Error::Store(
    ...
    match substring_hits.len() {
        0 => Err(hoangsa_memory_core::Error::Store(format!(
        ...
        _ => {
            ...
            if best_score >= 0.6 && best_score > second {
                ...
                let titles: Vec<String> = substring_hits
                    ...
                    .collect();
                Err(hoangsa_memory_core::Error::Store(format!(
